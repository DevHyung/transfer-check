...
worksheet.write_row(0,0,['카드번호','트랜잭션ID','사용자구분코드','출발ID','출발_호선','출발수단','도착ID','도착_호선','도착수단','환승시간(초)'])
row = 1
col = 0
...
for line in lines[1:]:
    ...
    if isFirst:
        ...
        if oldCardNo == newCardNo and oldTranjectionId == newTranjectionId: #환승
            ...
            a = [1, 2, 3, 4]
            worksheet.write_row(row,col, [oldCardNo, oldTranjectionId, datas[10], lines[firstIdx].split(',')[14], lines[firstIdx].split(',')[14],startCode, lines[lateIdx].split(',')[12],lines[lateIdx].split(',')[12],endCode,get_second(lines[firstIdx].split(',')[13],lines[lateIdx].split(',')[3])])
            row+=1
